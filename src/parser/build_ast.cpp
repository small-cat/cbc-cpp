...
void BuildAstVisitor::AddImportFile(const std::string& fname, ast::Location* loc) {
  ast::ImportFileNode* imp_node = node_tracker_.CreateInstance<ast::ImportFileNode>(fname, loc);
  import_files_.push_back(imp_node);
}

std::vector<ast::ImportFileNode *> BuildAstVisitor::import_files() {
...
antlrcpp::Any BuildAstVisitor::visitCompilation_unit(SesameParser::Compilation_unitContext * ctx) {
  ...
  for (auto& im : import_ctx) {
    auto loc = GetLocation(GetCFlatToken(im->start));
    auto import_fname_ctx = im->import_name();
    AddImportFile(tokens_->getText(import_fname_ctx), loc);
  }
...
antlrcpp::Any BuildAstVisitor::visitDeclaration_file(SesameParser::Declaration_fileContext * ctx) {
  ...
  for (auto& imp : imports_ctx) {
    ...
    auto loc = GetLocation(GetCFlatToken(imp->start));
    AddImportFile(fname, loc);
  }
